...
float density_s(Particle particle, Particle[] particlesContainer) {
    const int h = 1; 
    int p = 0; 
    for (Particle other_particle : particlesContainer) {
...
glm::vec3 compute_pressure(Particle particle, Particle[] particlesContainer) {
    ...
    for (Particle other_particle : åparticlesContainer) {
        ...
        if (r != 0) {
            const int n = (particle.pos - other_particles.pos)/r; 
            f_p += n * pressure; 
        }
    }
    return f_p; 
...
glm::vec3 compute_viscosity(Particle particle, Particle[] particlesContainer) {
    ...
    const int h = 1;
    const int mu =  0.01; // viscosity of water at 20 degrees
    for (Particle other_particle : *particlesContainer) {
...
glm::vec3 compute_surface_tension(Particle particle, Particle[] particlesContainer) {
    glm::vec3 ft = gml::vec3(0.0, 0.0, 0.0); 
    ...
    for(Particle other_particle : particlesContainer) {
        ...
        if(r != 0) {
          const float n = (particle.pos - other_particle.pos)/r; 
          c += surface_tension * n;
